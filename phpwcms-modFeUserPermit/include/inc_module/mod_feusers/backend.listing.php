...
if(isset($_POST['do_pagination'])) {
	...
	$_SESSION['filter_feuserpermit'] = clean_slweg($_POST['filter']);
	if(empty($_SESSION['filter_feuserpermit'])) {
		unset($_SESSION['filter_feuserpermit']);
	} else {
		$_SESSION['filter_feuserpermit'] = convertStringToArray($_SESSION['filter_feuserpermit'], ' ');
		$_POST['filter'] = $_SESSION['filter_feuserpermit'];
	}
...
$_entry['query'] .= " AND detail_regkey="._dbEscape(MODULE_REGKEY);

if(isset($_SESSION['filter_feuserpermit']) && is_array($_SESSION['filter_feuserpermit']) && count($_SESSION['filter_feuserpermit'])) {

	$_entry['filter_array'] = array();

	foreach($_SESSION['filter_feuserpermit'] as $_entry['filter']) {
		//$_entry['filter_array'][] = "CONCAT(detail_firstname, detail_lastname, detail_company, detail_city, detail_zip, detail_email, detail_int1, detail_int2) LIKE '%".aporeplace($_entry['filter'])."%'";
		$_entry['filter_array'][] = "detail_login LIKE '%".aporeplace($_entry['filter'])."%'";
	}
	if(count($_entry['filter_array'])) {

		$_SESSION['filter_feuserpermit'] = ' AND ('.implode(' OR ', $_entry['filter_array']).')';
		$_entry['query'] .= $_SESSION['filter_feuserpermit'];

	}

} elseif(isset($_SESSION['filter_feuserpermit']) && is_string($_SESSION['filter_feuserpermit'])) {
...
// paginating values
$_entry['count_total'] = _dbQuery('SELECT detail_id FROM '.DB_PREPEND.'phpwcms_userdetail WHERE '.$_entry['query'], 'COUNT');
...
<div class="navBarLeft imgButton chatlist">
	&nbsp;&nbsp;
	<a href="<?php echo MODULE_HREF ?>&amp;edit=0" title="<?php echo $BLM['create_new'] ?>"><img src="img/famfamfam/user_add.gif" alt="Add" border="0" /><span><?php echo $BLM['create_new'] ?></span></a>
</div>
...
$sql  = 'SELECT ud.*, ac.acat_name, ac.acat_aktiv, ac.acat_regonly FROM '.DB_PREPEND.'phpwcms_userdetail ud ';
$sql .= 'LEFT JOIN '.DB_PREPEND.'phpwcms_articlecat ac ';
$sql .= 'ON ud.detail_int1=ac.acat_id ';
...
if($data) {
	...
	foreach($data as $row) {
		...
		echo '<img src="img/famfamfam/user.gif" alt="'.$BLM['florist_entry'].'" /></td>'.LF;

		echo '<td class="dir" width="55%">&nbsp;'.html_specialchars($row["detail_login"])."</td>\n";
		echo '<td class="dir nowrap" width="35%">&nbsp;'.html_specialchars($row["acat_name"]);
		if(!$row["acat_aktiv"] || $row["acat_regonly"]) {
			$row['status'] = array();
			if(!$row["acat_aktiv"]) {
				$row['status'][] = $BL['be_inactive'];
			}
			if($row["acat_regonly"]) {
				$row['status'][] = $BL['be_locked'];
			}
			if(count($row['status'])) {
				echo ' ('.implode(', ', $row['status']).')';
			}
		}
		echo "&nbsp;</td>\n";

		echo '<td width="10%" align="right" nowrap="nowrap" class="button_td">';
		echo '<a href="'.MODULE_HREF.'&amp;edit='.$row["detail_id"].'"><img src="img/button/edit_22x13.gif" border="0" alt="" /></a>';
		...
		echo '" title="delete: '.html_specialchars($row["detail_login"]).'"';
		echo ' onclick="return confirm(\''.$BLM['delete_entry'].' '.js_singlequote($row["detail_login"]).'\');">';
